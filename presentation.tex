...
\frametitle{}
\begin{tikzpicture}
\begin{axis}[enlargelimits=false,ticks=none]
\addplot3[fill=blue]
coordinates{
(1,0,0)
(1,1,0)
...
(3,0,1)
    ...
    %\addplot3[name path=toppath,fill=blue, opacity=0.1, fill opacity=0.4,samples=2] (x-1,x,0);
    %\addplot3[name path=botpath,fill=blue, opacity=0.1, fill opacity=0.4,samples=2] (x-1,x,0);
    %\addplot [gray] fill between[of=toppath and botpath];
  \end{axis}
%  
\draw [black, thick] (1,3,0) -- (3,3,1.5);
\end{tikzpicture}
\end{frame}
%------------------------------------------------------------------------------
\begin{frame}
